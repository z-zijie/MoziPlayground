...
rm -rf docs/
rm -rf .codex-create-prd-result.txt
rm -rf .codex-review-prd-result.yaml
...
codex exec \
  ...
  --skip-git-repo-check \
  --output-last-message .codex-create-prd-result.txt \
  - <<'PROMPT'
...
codex exec \
  ...
  --dangerously-bypass-approvals-and-sandbox \
  --skip-git-repo-check \
  --output-last-message .codex-review-prd-result.yaml \
  - <<PROMPT
...
with open(".codex-review-prd-result.yaml", "r", encoding="utf-8") as f:
